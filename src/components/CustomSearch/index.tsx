...
import { ChatContextValue, useChatContext } from 'stream-chat-react';
import "../../styles/CustomMessage.css";
import {ReactComponent as CloseIcon}  from '../../assets/close.svg';
interface CustomSearchProps {}

const CustomSearch: React.FC<CustomSearchProps> = (): React.JSX.Element => {
    const { client }:any = useChatContext() as ChatContextValue;
        ...
        const searchDocuments = async () => {
            switch (type) {
                ...
                case 'message':
                    ...
                    const detailedResults = await Promise.all(searchResults1.results.map(async (message:any) => {
    ...
    const ChannelSearchResultPreview = ({ channel }:any) => {
    ...
    const UserSearchResultPreview = ({ user }:any) => {
    ...
    const MessageSearchResultPreview = ({ message }:any) => {
    ...
    const SearchResultsPreview = ({ searchResults }:any) => {
        const type = searchParams.get('type');
        ...
        return (
            <ul className='search-results'>
                ...
                {type === 'channel' &&
                    searchResults.items.map((item:any) => <ChannelSearchResultPreview key={item.cid} channel={item} />)}
                {type === 'user' &&
                    searchResults.items.map((item:any) => <UserSearchResultPreview key={item.id} user={item} />)}
                {type === 'message' &&
                    searchResults.items.map((item:any) => <MessageSearchResultPreview key={item.id} message={item} />)}
            </ul>
        );
    };
